...
public class AlmanacPanel : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler
{
    ...
    public class ElementView : GridView
    {
        ...
        public void SetSelectedColor(Color color)
        {
            ...
            for (var index = 0; index < elements.Count; ++index)
            {
                ...
                element.SetSelectedColor(color);
            }
        }
        public override void OnSearch(string query)
        {
    ...
    public class InfoView
    {
        ...
        public class Icons : InfoElement
        {
            ...
            public void SetIcons(params AchievementManager.Achievement[] achievements)
            {
                ...
                for (int index = 0; index < elements.Count; ++index)
                {
                    ...
                    if (achievements.Length > index)
                    {
                        ...
                        element.SetIcon(achievement.icon);
                        element.SetName(achievement.Name);
                        element.SetAmount(string.Empty);
                        element.SetIconColor(achievement.IsCompleted(Player.m_localPlayer) ? Color.white : Color.black);
                        element.OnClick(() => achievement.OnClick(instance, null));
